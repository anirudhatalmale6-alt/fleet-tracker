...
router.post('/', authenticate, requireRole('admin', 'staff'), (req, res) => {
  const { truck_id, customer_name, origin, destination, distance_km, fuel_litres, fuel_cost, trip_date, notes } = req.body;
  ...
  const result = db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(truck_id, req.user.id, customer_name, origin, destination, distance_km, fuel_litres, fuel_cost || 0, trip_date, notes || null);
...
router.put('/:id', authenticate, requireRole('admin'), (req, res) => {
  ...
  const { truck_id, customer_name, origin, destination, distance_km, fuel_litres, fuel_cost, trip_date, notes } = req.body;
  db.prepare(`
    UPDATE trips SET truck_id=?, customer_name=?, origin=?, destination=?, distance_km=?, fuel_litres=?, fuel_cost=?, trip_date=?, notes=?
    WHERE id=?
  `).run(
    ...
    fuel_cost !== undefined ? fuel_cost : trip.fuel_cost, trip_date || trip.trip_date,
...
router.get('/export/csv', authenticate, (req, res) => {
  ...
  let sql = `
    SELECT tr.trip_date, t.plate, tr.origin, tr.destination, tr.distance_km, tr.fuel_litres, tr.fuel_cost, tr.customer_name, u.name as logged_by
  ...
  const header = 'Date,Truck,From,To,Distance (km),Fuel (L),Fuel Cost (₦),Customer,Logged By\n';
  const csv = header + rows.map(r =>
    `${r.trip_date},"${r.plate}","${r.origin}","${r.destination}",${r.distance_km},${r.fuel_litres},${r.fuel_cost},"${r.customer_name}","${r.logged_by}"`
  ).join('\n');
